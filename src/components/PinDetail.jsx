      ...
      <div className='flex xl:flex-row flex-col m-auto bg-white' style={{ maxWidth: '1500px', borderRadius: '32px' }}>
        <div className="flex justify-center items-center md:items-start flex-initial">
          <img src={pinDetail?.image && urlFor(pinDetail?.image).url()}
            className='rounded-t-3xl rounded-b-lg' alt='pin detial image'
          />
        </div>
        <div className="w-full p-5 flex-1 xl:min-w-620">
          ...
          <div>
              ...
              {pinDetail?.comments?.map((comment, i) => (
                <div className="flex gap-2 mt-5 items-center bg-white rounded-lg" key={i}>
                  <img src={comment?.postedBy?.image}
                    alt='user profile' className='w-10 h-10 rounded-full cursor-pointer' />
            ...
            <div className="flex flex-wrap mt-6 gap-3">
              ...
              <button type='button' className='bg-red-500 text-white px-6 rounded-full py-2 font-semibold text-base outline-none'
                onClick={addComment}
              >
